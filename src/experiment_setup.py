...
def main():      
    '''
    Either pass reference experiment ID or train/test splits
    '''
    ...
    parser.add_argument('--in_context', type=int)
    ...
    if args.reference_experiment_id > 0:
        ...
        summary['train'] = ref_exp['train']
        summary['test'] = ref_exp['test']
        info['train'] = summary['train']
        info['test'] = summary['test']

        exp_dir = os.path.join(exp_home, args.dataset, args.model_size, args.method, 
                f"train{summary['train']}_test{summary['test']}_id{exp_id}") 
